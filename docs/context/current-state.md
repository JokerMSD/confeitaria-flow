...
Modulos parcialmente evoluidos: clientes e usuarios ja possuem base de schema e rotas dedicadas; a tela de cliente agora mostra historico de pedidos e pode iniciar novo pedido contextualizado, mas a autenticacao ainda nao saiu de `AUTH_USERS_JSON`; loja publica e fluxo de cliente final ainda nao foram abertos como produto real; previsao de producao por lote/turno segue pendente.
...
Proximo ponto natural do sistema: consolidar o vinculo real entre clientes e pedidos tambem no cadastro/edicao de pedido, manter pedidos/estoque/fila como base estavel e abrir novos modulos apenas quando o contrato backend do dominio anterior estiver coerente.
